...
"""
    ...
    command_string = f"pytest {test_files} --doctest-modules --cov=src/panel_components"
...
@task()
def pylint(command, files="setup.py tasks src/panel_components tests"):
...
@task
def mypy(command, files="setup.py tasks src/panel_components tests"):
